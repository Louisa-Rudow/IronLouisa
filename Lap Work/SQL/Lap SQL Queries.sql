...
select district_id, client_id from client where district_id = 72 order by client_id desc limit 1
...
select date, duration, count(loan_id) from loan where date like "9712%" group by date, duration order by date, duration
...
select account_id, type as transaction_type, round(sum(amount)) as total_amount from trans where account_id=396 
group by type order by transaction_type 
UPDATE trans set transaction_type= "INCOMING" where "PRIJEM"
